...
QSOCapture is a lightweight contest audio recorder and log player for amateur radio operators. It captures logged QSOs from **N1MM Logger+** (UDP broadcast) or **N3FJP software** (TCP API), slices out audio for each contact, and presents everything in a clean, color-coded web dashboard.
...
7. [Logging software integration](#7-logging-software-integration)
   - [7a. N1MM Logger+ Integration](#7a-n1mm-logger-integration)
   - [7b. N3FJP software](#7b-n3fjp-software-ac-log--contest-logs)
...
- **Type = Logger QSOs** – lets you select the saved contests, look up a specific callsign, or use the other filter options. Contest names are generated by N1MM and simplified to a readable form (for example `2025 CQWW CW`).
### 5. Play and save
...
| **N1MM/N3FJP badge** | Status of the active logger listener (N1MM `listening` or N3FJP `connected`). |
...
- **Logger QSOs** – list of sliced contacts showing Timestamp, Call, Band, Mode, Freq, Exch, RX, Contest (in a readable format, e.g. `2025 CQWW CW`) and an inline audio player.
...
| **Logger source** | Logging program that drives QSO capture: `n1mm` (N1MM UDP broadcast) or `n3fjp` (N3FJP TCP API). |
| **N1MM address (bind_ip:udp_port)** | UDP address and port for receiving N1MM Logger+ broadcasts (default `127.0.0.1:12060`). Shown when **Logger source = n1mm**. |
| **N3FJP address (host:port)** | N3FJP TCP API host and port (default `127.0.0.1:1100`). Shown when **Logger source = n3fjp**. |
| **N3FJP contest name** | Optional override for the contest label stored with each N3FJP recording (blank = `GENERAL`). |
| **N3FJP reconcile interval (s)** | How often QSOCapture compares the N3FJP QSO list with its database to catch edits/deletes (default `60`). |
| **N3FJP list window** | How many of the most recent N3FJP contacts are compared during reconciliation (default `100`). |
...
## 7. Logging software integration

QSOCapture captures QSO metadata from exactly one logging program at a time,
chosen with the **Logger source** setting: **N1MM Logger+** (UDP broadcast) or
**N3FJP software** (TCP API).

### 7a. N1MM Logger+ Integration
...
4. In QSOCapture settings, set **Logger source** to `n1mm` and confirm the
   **N1MM address** port matches (default `127.0.0.1:12060`).
...
### 7b. N3FJP software (AC Log / contest logs)

N3FJP programs (Amateur Contact Log and the contest loggers) expose a **TCP API**
on port **1100** by default. QSOCapture connects to them as a client.

#### Configuration

1. In your N3FJP program, open **Settings → Application Program Interface** and
   check **TCP API Enabled** (server). Note the port (default **1100**).
2. In QSOCapture settings set **Logger source** to `n3fjp`.
3. In the **N3FJP address (host:port)** field enter the host and port of the
   N3FJP computer (default `127.0.0.1:1100`).
4. **N3FJP contest name** – optional name that overrides the DEFAULT contest
   label stored with each recording (blank = `GENERAL`).

#### How it works

After connecting, QSOCapture enables all-update notifications
(`SETUPDATESTATE TRUE`). When the operator logs a contact in N3FJP software, the
program receives an `<ENTEREVENT>` and slices the audio the same way as for
N1MM (`pre_roll`/`post_roll`; filename contains date, time, callsign, band and
receiver label). QSO details are stored in the same SQLite database, stamped
with source `n3fjp`.

#### Editing and deleting QSOs in N3FJP

Because the N3FJP API only sends a generic `EDITDELETEEVENT` (without a record
id), QSOCapture periodically reconciles its database against the logger's
`LIST` (most recent contacts):

* **Metadata edit** (same call/date/time/band/mode) – fields are refreshed in
  place and the audio file keeps its name.
* **Identity edit** (call or band/time changed, single unambiguous candidate) –
  the audio file is renamed to the new callsign/band and DB record updated.
* **Delete** – the DB row and the audio file are removed.

Safety: if a record cannot be matched 1:1, QSOCapture leaves it unchanged and
logs a warning. The reconcile is triggered by the event plus a periodic tick
(**N3FJP reconcile interval**) and compares the newest **N3FJP list window**
records.
...
| **Type** | Switch between **Logger QSOs** and **Continuous** recordings. |
